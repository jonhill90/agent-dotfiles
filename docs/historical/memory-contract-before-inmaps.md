...
# Memory contract before INMAPS

Superseded by [Memory](../canonical/memory.md) on 2026-09-06. Preserved as history.
...
- **Schema inside the vault** (v2, 2026-07-12 — an OKF v0.1-conformant
  ...
  - `01 - Notes/<12-digit-id>.md` — one fact per note, **semantic slug
...
E06), but `settings/default-skills.txt` was then a single flat roster,
so Claude Code, Codex, and Pi — which passed both scenarios on the
canonical instructions alone — loaded them too. (The sectioned format
below is the answer to this; it landed in P2-M4 and was used by
`sanity-check` until 2026-07-29, when that skill moved to public opt-in for
failing §10.1 rule 5. The mechanism is enforced on all four harnesses and
currently has no user.) Unclosed, every rung-2
...
- **Copilot:** `disabledSkills` in `~/.copilot/settings.json` — resolved
  ...
  harnesses share, so Tier A cannot separate it from Codex and Pi; the
  Tier B key is what makes a scoped roster real there. Three earlier
  checks recorded the surface absent because `copilot help config` does
  not document the key.
...
back affirmative for Claude Code and Pi (2026-07-26) and V10 for Copilot
(2026-07-27), so **Tier B covers all four harnesses** and the case for
Tier C is weaker than when this was written, not stronger. Its last
motivation was Copilot having no disable surface; Copilot has one. Tier C
is now reachable only if a measured §6 budget overrun cannot be fixed by
scoping the roster.
...
**Built 2026-07-30 (jonhill90/skills#96).** The roster expresses it as `@name-only`,
...
- `sync doctor` — environment checks. Actually implemented: `apm` on PATH,
  the Cowork skills-dir pin, `AGENT_MEMORY_VAULT` set and personal, the MCP
  auth var, each harness root file present, and shared-path drift and
  substitution. **Not** checked, despite earlier drafts of this section
  claiming otherwise: node, the Obsidian CLI, Pi, and trust state —
  `install.sh` gates on this command, so a machine missing those still
  passes (env vars checked: `AGENT_MEMORY_VAULT`,
...
3. Prompt once for `AGENT_MEMORY_VAULT` → write a marker-fenced
   shell-profile block, and source `~/.zshrc.local` if it exists.
   **MCP env vars are not prompted for and no override file is created** —
   set them yourself; `doctor` reports the missing one.
...
**Ownership (2026-08-09, #10):** the scenarios, counter-scenarios, harness
runner, results/transcripts, scoring (`eval_score.py`) and arming
(`eval_arm.py`) tools, and the paths named `tests/evals/...` below now live
in the private jonhill90/agent-evals (evidence unavailable publicly)
repository, not in this one. The `tests/evals/...` paths in this section
describe that repository's layout — this repository no longer contains
them. `scripts/measure_e15.py` and `scripts/measure_context.py` (§10
static-context acceptance) stayed here: they measure this repository's own
deployed footprint, not scored behavior.
...
- Release-required columns: **all four first-class harnesses** — Claude
  Code, Codex, Copilot and Pi — since P2-M3 (2026-07-18); breakage on any
  one blocks release. Model variation within a column is the secondary
  matrix; record partial coverage honestly when accounts are unavailable.
...
to fix bugs in the harness (results — private repository jonhill90/agent-evals, evidence unavailable publicly (`tests/evals/results/2026-07-26-p2m5-counter-pass1.md`)).
...
(pass 2 — private repository jonhill90/agent-evals, evidence unavailable publicly (`tests/evals/results/2026-07-26-p2m5-counter-pass2.md`)). Adoption rows in the provenance manifest
...
failures. They were re-verified at the bar above in P2-M5, which
closed 2026-07-27: both clear the ×3 adoption bar on Copilot and both
...
| V10 | Copilot per-skill disable surface | **Resolved 2026-07-27, affirmative** (hands-on; §4.1 Tier B): `disabledSkills` in `~/.copilot/settings.json` — the file the wrapper already manages. `copilot skill list --json` reports `enabled: false` for a listed skill and a fresh `copilot -p` no longer lists it (evidence — private repository jonhill90/agent-evals, evidence unavailable publicly (`tests/evals/results/2026-07-27-copilot-column.md`)). The three prior checks (1.0.70, 1.0.71, 1.0.75) recorded it absent because `copilot help config` omits the key and `/skills` was read as session-only state; the setting it writes is persistent. **All four first-class harnesses now have a Tier B surface**, and `sync.copilot_disabled_skills()` derives the list from the roster |
