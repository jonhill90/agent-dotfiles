...
| Kept tool-skill roster (`gh-cli`, `az-devops`, `linear`, `obsidian`, `using-tmux`, `create-skill`) | Jon (authored) | Adopt (roster cut 2026-07-12) | Daily drivers; each tool skill has acceptance checks in `tests/evals/acceptance/`; displaceable only per the two-track rule |
