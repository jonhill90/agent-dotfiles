...
if hook_command_violates "$RULE" "self-close:$OWN_ISSUE"; then
  hook_block "$RULE" \
    "this closes issue #$OWN_ISSUE, and the current branch '$BRANCH' is dispatched against that same issue. A lane never closes its own issue -- that certification belongs to whoever reviews the PR, not the lane that wrote it."
fi
