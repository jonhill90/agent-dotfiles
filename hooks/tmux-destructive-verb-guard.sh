...
echo "$HOOK_COMMAND_PREFIX" | grep -qE "$DESTRUCTIVE_RE" || exit 0
...
if ! echo "$HOOK_COMMAND_PREFIX" | grep -qE 'TMUX_TMPDIR='; then
  hook_block "$RULE" \
...
if ! echo "$HOOK_COMMAND_PREFIX" | grep -qE '(^|[;&|]|\s)env\s+-u\s+TMUX\b'; then
  hook_block "$RULE" \
