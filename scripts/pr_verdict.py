...
current APPROVE — this repository's own port of the comment-verdict gate
`jonhill90/agent-supervisor` already runs (`scripts/supervisor/verdict.py`,
`scripts/supervisor/verdict-independence.sh`), following the shape
`jonhill90/skills#255` already adapted rather than re-deriving it from
`agent-supervisor` a third time (`jonhill90/agent-tui#107` is the other
sibling port, in Go rather than Python — this file follows `skills#255`
because this repository, like `skills`, is Python with a stdlib
`unittest` `tests/` suite; `agent-tui#107`'s shape does not transfer).
...
repository read it before this file (measured: `git grep -i
'Reviewed-SHA'` on `origin/main` before this change returned nothing
here, the last of the estate's four Python/Go repos without a reader —
`agent-supervisor` has the original, `skills` and `agent-tui` both have
ports). This script is that reader.
...
exclusion) is copied here close to verbatim from `skills#255`'s own
`scripts/pr_verdict.py`, which itself ported it near-verbatim from
`agent-supervisor` — it is pure text processing, proven against real
reviewer prose across a dozen-plus `agent-supervisor` issues (#53, #192,
#196, #198, #213, #232, #475 — each cited inline below at the rule it
fixed), and re-deriving it worse from scratch a third time would throw
that history away. What is NOT ported is `verdict-independence.sh`'s
lane-identity resolution (`author_lane_for`, `lane_relation`,
`resolve_lane_relation`): that machinery exists to answer "which lane
authored this PR" from a tmux supervisor ledger this repository does
not have and must not grow one to get — this repository's own scope is
personal harness configuration, not a session ledger.
...
in the PR's own body — identical to `skills#255`'s own adaptation.
There is no ledger here to resolve authorship independently, so
authorship is self-declared -- the same trust model `Review-Lane:`
already has (a lane names itself; nothing here cross-checks the name
against a registry). That is not a weaker guarantee than the reviewer
side already carries, only a plainly-stated one: a PR with no
`Author-Lane:` trailer has unknown authorship, and this script refuses
to call ANY verdict on it independent -- fail closed, never a guess,
the same posture every source in `verdict.py` already takes for a
...
`scripts/pr_ci_state.py`'s own exit-code contract in this repository:
  ...
  3  unknown    -- something IS on record but this script cannot call it
                   ...
                   see `independence_verdict` in `agent-supervisor`'s
...
# via jonhill90/skills#255's own scripts/pr_verdict.py (its own docstring
# names the agent-supervisor issue each rule fixed; kept here so a reader
# auditing THIS file does not have to go find either other repo).
# ---------------------------------------------------------------------------
...
def _default_gh_pr_view(repo: str, number: int) -> dict:
    """The one network call this script makes. A function-typed seam so
    tests supply a fake payload instead of a real `gh` subprocess --
    `scripts/pr_ci_state.py`'s own `run_gh`/`gh_json` seam, matched here as
    a single argument default rather than a module-level runner because
    `resolve()` needs only this one call."""
    raw = subprocess.run(
