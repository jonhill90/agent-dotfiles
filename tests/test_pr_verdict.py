"""Mutation-check for pr_verdict.py's comment-verdict gate -- the same
"a guard nobody has watched fail is not a guard" bar this repository's
other mechanical gates apply (`test_pr_ci_state.py`), applied here to
the four directions this gate must resolve correctly:
...
repository's other scripts/*.py test files' synthetic-fixture discipline.

Ported from `jonhill90/skills#255`'s `tests/test_pr_verdict.py` alongside
`scripts/pr_verdict.py` itself -- see that module's own doc comment for
why this repo followed `skills#255`'s shape rather than `agent-tui#107`'s."""
...
def _resolve(payload):
    return pv.resolve("jonhill90/agent-dotfiles", 1, gh_pr_view=lambda repo, number: payload)


class MutationCheckFourDirections(unittest.TestCase):
    """The exact four cases this gate must resolve, each demonstrated in
    the direction that matters -- a false PASS is the expensive one, so
    every failing case here is checked for the SPECIFIC reason, not just a
    non-zero result, so a future change that returns 'unknown' for the
    wrong reason still gets caught."""
...
class BlankReviewLaneSelfApprovalBypass(unittest.TestCase):
    """Security regression, filed as jonhill90/agent-dotfiles#304's own
    unresolved REQUEST CHANGES and fixed here: a BLANK `Review-Lane:`
    value let `_REVIEW_LANE_LINE_RE`'s post-colon `\\s*` consume the line
    break and capture the NEXT line's text (`Reviewed-SHA: ...`) instead
    of matching empty. That non-empty garbage never equals a real
    ...
    blank `Review-Lane:` trailer got treated as a valid, different
    reviewer and the PR resolved `approved`. Ported from
    `jonhill90/skills#260`'s identical regression test. This exact shape
    -- blank trailer, same lane as Author-Lane, real head SHA on the very
    next line -- must never again resolve to anything but `unknown`."""
    ...
    def test_blank_review_lane_does_not_capture_the_next_line(self):
        ...
        # outcome -- pins the exact failure mode so a future refactor of
        # the pattern can't silently reopen it while still passing the
        # resolve()-level test above by coincidence.
    ...
    def test_main_returns_zero_for_approved(self):
        ...
        try:
            with contextlib.redirect_stdout(io.StringIO()):
                rc = pv.main(["--repo", "jonhill90/agent-dotfiles", "--number", "1"])
        finally:
